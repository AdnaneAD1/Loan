...
export default function SendMoney() {
    ...
    const handleSendMoney = async () => {
        ...
        try {
            ...
            const { transaction_id, pendingTransaction } = response.data;

            if (pendingTransaction) {
                const confirmAction = window.confirm('Vous avez une transaction en cours. Ok pour continuer ou annuler pour annuler la transaction en cour ?');
                if (confirmAction) {
                    router.push(`/codetransaction?transaction_id=${transaction_id}`);
                } else {
                    await axios.post(`/api/annuler-transaction/${transaction_id}`);
                    alert('Transaction annulée.');
                }
            } else {
                alert(`Vous voulez envoyer €${amount} à ${formData.beneficiaryName}. Veuillez contacter l'admin pour obtenir les codes nécessaires à la finalisation de la transaction.`);
                router.push(`/codetransaction?transaction_id=${transaction_id}`);
            }
        } catch (error) {
            alert('Erreur lors du retrait.');
